...
\
# =============================================================================\
# DATA FUNCTIONS (Reused from notebook)\
# =============================================================================\
\
@st.cache_data(ttl=3600)\
def fetch_wrds_data(tickers, _username, _password):\
    """Cached WRDS data fetcher"""\
    try:\
...
def render_risk_card(ticker, metrics):\
    ...
    <div class="\{color_class\}">\
        ...
        \{'\uc0\u9888 \u65039  WARNING: Risk of entering Distress Zone within 3 years!' if metrics['worsening'] else '\u9989  Trend is stable or improving'\}\
    </div>\
    """, unsafe_allow_html=True)\
\
...
\
def main():\
    st.title("\uc0\u9888 \u65039  US Listed Company Default Risk Predictor")\
    ...
    with st.sidebar:\
        ...
        data_mode = st.radio(\
            "Data Mode",\
            ["Live WRDS (Requires Account)", "Demo Mode (Pre-loaded Data)"],\
        ...
        if data_mode == "Live WRDS (Requires Account)":\
            ...
            st.info("\uc0\u8505 \u65039  Your credentials are not stored; used only for this session")\
        \
        st.divider()\
        \
        # Stock Input\
        ...
        \
        analyze_btn = st.button("\uc0\u55357 \u56589  Analyze Risk", type="primary", use_container_width=True)\
        \
        st.divider()\
        st.caption("Powered by: WRDS Compustat | Altman Z-Score | Linear Forecasting")\
        st.caption("\uc0\u9888 \u65039  Disclaimer: Academic demo only; not investment advice.")\
    \
    # Main content area\
    if analyze_btn:\
        ...
        if len(tickers) > 5:\
            ...
            tickers = tickers[:5]\
        \
        # DEMO MODE: Load from CSV if available\
        if data_mode == "Demo Mode (Pre-loaded Data)":\
            try:\
                demo_data = pd.read_csv('z_score_analysis.csv')\
                demo_data['datadate'] = pd.to_datetime(demo_data['datadate'])\
                data_available = True\
            except FileNotFoundError:\
                st.error("Demo data not found. Please run the analysis notebook first or switch to Live mode.")\
                return\
        else:\
            demo_data = pd.DataFrame()\
            if not wrds_username or not wrds_password:\
                st.error("Please enter WRDS credentials for Live mode")\
                return\
        \
        # Progress bar\
        ...
        # Display Results\
        st.divider()\
        st.header("\uc0\u55357 \u56522  Risk Analysis Results")\
        \
        # Summary table\
        summary_data = []\
        for r in results:\
            ...
            summary_data.append(\{\
                ...
                'Trend': "\uc0\u55357 \u56521  Declining" if m['slope'] < -0.1 else "\u55357 \u56520  Improving" if m['slope'] > 0.1 else "\u10145 \u65039  Stable"\
            \})\
        \
        ...
        for r in results:\
            ...
            with col1:\
                ...
                st.metric("Model Reliability (R\'b2)", f"\{m['r2']:.2f\}")\
                \
                # Investment recommendation\
                if m['current_zone'] == 'Distress' or m['worsening']:\
                    st.error("\uc0\u55357 \u56628  **High Risk**: Consider divestment or hedging")\
                elif m['current_zone'] == 'Grey':\
                    st.warning("\uc0\u55357 \u57313  **Monitor Closely**: Review quarterly filings")\
                else:\
                    st.success("\uc0\u55357 \u57314  **Lower Risk**: Maintain current position")\
            \
            with col2:\
        ...
        # Export option\
        st.download_button(\
            label="\uc0\u55357 \u56549  Download Full Report (CSV)",\
...
\
        ...
        st.info("\uc0\u55357 \u56392  Enter stock tickers in the sidebar and click 'Analyze Risk' to begin")\
        \
